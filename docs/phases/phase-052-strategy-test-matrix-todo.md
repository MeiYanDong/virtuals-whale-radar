...
- [x] 确认卖出策略口径：税率 `<=30%` 后，收益率策略和大额买入策略独立记账，各自最多控制总仓位 `50%`，同时触发合并为一笔卖出。
- [x] 加固余额不足保护：真实余额低于目标卖出数量时，只按实际卖出的原始仓位比例更新策略状态。
- [x] 回测结果：SR 卖出 `2` 次，最终 `+43.5457%`，低于纯持有；ISC 卖出 `2` 次，最终 `+41.4107%`，高于纯持有。
- [ ] 如需进入生产自动卖出，下一步在 Phase 053 接入执行账本状态、真实余额读取、sell simulation 和 broadcast gate。
