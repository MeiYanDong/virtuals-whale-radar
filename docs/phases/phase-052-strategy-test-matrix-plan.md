...
- 策略口径：
  ...
  - 进入卖出观察：税率 `<=30%`。
  - 收益率策略独立控制最多 `50%` 总仓位：我的收益率 `>=30%` 卖 `30%`，`>=50%` 卖 `50%`。
  - 大额买入策略独立控制最多 `50%` 总仓位：单笔买入 `>=5,000 VIRTUAL` 卖 `30%`，`>=8,000 VIRTUAL` 卖 `50%`。
  - 两条策略独立记账；同一刻同时触发时合并为一笔卖出交易。
  - 程序按原始累计收到 token 数量计算卖出比例，避免按当前余额重复折扣。
  - 若真实余额低于目标卖出数量，只按实际卖出的原始仓位比例更新策略状态，避免误记已卖额度。
- SR/ISC 回测结果：
  - SR：卖出 `2` 次，最终 `+54.432149 V / +43.5457%`，低于纯持有 `+68.9064%`；第一次在 tax `27` 同时触发收益率与 `5,535 V` 大买入，合并卖出 `60%`。
  - ISC：卖出 `2` 次，最终 `+144.937286 V / +41.4107%`，高于纯持有 `+38.9383%`；仅由收益率策略触发。
- 当前边界：
  - 该模块只完成策略决策和历史回测。
  - 尚未接入生产自动卖出服务、执行账本状态持久化和真实 sell broadcast。
