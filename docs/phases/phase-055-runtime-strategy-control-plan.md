...
- `fdv_limit_enabled`：是否启用含税估算 FDV 限价，默认关闭。
- `fdv_limit_wan_usd`：最高含税估算 FDV，单位 `万 USD`；开启后只在当前含税估算 FDV 小于等于该值时允许买入。
- `max_buy_v`：单笔最大买入上限。
...
- API 静态校验：
  ...
  - `fdv_limit_enabled=true` 时，`fdv_limit_wan_usd > 0`
...
- 前端重构为“买入策略卡”，而不是普通参数表：
  ...
  - `估算市值限价`：可选保险条件，默认关闭；开启后不改变固定入场门槛，只额外限制含税估算 FDV 不能高于管理员设定值。
  - `风险上限`：编辑单笔上限和项目预算。
- 买入触发条件按当前后端策略事实展示：
  - `LIVE 中`
  - `税率 ≤ 95%`
...
- 含税估算 FDV 限价默认关闭时不改变现有 SR/ISC 回放结果；开启后高于限价的信号必须被跳过，低于或等于限价的信号继续按原策略买入。
