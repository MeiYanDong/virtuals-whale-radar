...
- [x] 新增管理员读取配置 API。
- [x] 新增管理员保存配置 API。
- [x] 校验基础买入、抄底买入、单笔上限和大户榜单预算。
- [x] 返回各策略当前已发送买入 V，防止对应预算下调到已执行金额以下。
...
- [x] 2026-05-18 TDS 100x 完整窗口模拟通过：99 个 tick 约 1 分钟跑完，产生 `0.1/0.2/0.1/0.2 VIRTUAL` 四次 paper buy intent，大户榜单预算 `0.6 VIRTUAL` 生效后阻断后续普通策略意图。
