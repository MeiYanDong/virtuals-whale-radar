...
- `backfill_last_scanned_block`

说明：

- 这是不同角色之间共享轻量状态的重要通道
- `last_processed_block` 表示 writer 已入库事件的最高区块；rolling backfill 使用独立的 `backfill_last_scanned_block` 作为扫描游标，避免 realtime/writer 入库把回扫游标推过头。
