...
- 2026-05-01 生产最终验收：
  ...
  - `writer / realtime / backfill / SignalHub` 均为 running；`vwr-backup.service` 是 oneshot，手动执行成功后回到 `inactive dead` 属正常状态。
...
- 已新增 SR 消融测试工具：`scripts/ops/sr_strategy_ablation_suite.py`。
- 已新增测试记录：`docs/sr-strategy-ablation-suite-2026-05-07.md`。
- 该 suite 明确区分：
  - 一个变量：只看税率、只看榜单 V、只看 FDV 成本条件。
  - 固定变量梯度：固定 `tax<=92 + fdv` 或 `tax<=95 + fdv` 时，测试 `50k/60k/70k/80k/90k/100k/110k/120k`。
  - 取消变量：取消榜单 V、取消税率、取消 FDV 成本条件。
  - 多变量组合：`spent x tax`，分别保留/取消 FDV 成本条件。
- 用户点名的关键结果：
  - `70k / 80k / 90k + tax<=95 + fdv`：三者在 SR 上首买都发生在 `boardSpentV=98,430.254 / tax=93 / 含税 FDV=340.432566 万 USD / 榜单成本=341.684717 万 USD`，模拟投入 `100V`，收益约 `+42.1103%`。
  - `100k + tax<=95 + fdv`：首买推迟到 `boardSpentV=132,032.472 / tax=92`，收益约 `+38.0501%`。
...
## 48. 自用实盘盈利闭环（Phase 061，2026-05-20）

当前主线从“Base 生态外部分发”切回“我们自己使用工具赚到钱”。Base 入口、钱包登录和 Base USDC Billing 保留为已完成能力，但短期不继续投入上架材料、比赛提交、外部用户体验打磨或用户授权自动交易。

### 48.1 暂停方向

- 暂停 Base 生态上架 / Base App / 比赛提交。
- 暂停用户提前授权自动交易设计。
- 暂停面向外部用户的欢迎页、登录页、Billing 体验继续优化。
- 暂停普通用户自定义买卖单和用户钱包自动执行。

### 48.2 当前目标

核心目标是证明产品能服务自用实盘盈利闭环：

1. 发现值得跟踪的 Virtuals 发射项目。
2. 在发射前完成项目配置、执行服务启动、预热和风险检查。
3. 发射窗口内稳定读取税率、FDV、大户榜单、钱包消耗和交易事件。
4. 自动买入链路能按当前策略触发、simulation、broadcast、receipt 和 fuse。
5. 自动卖出链路能按持仓、税率、大额买入和收益条件触发 approve/sell/receipt。
6. 项目结束后自动归档 samples、events、execution ledger、fuses 和 summary。
7. 复盘每次真实窗口的买入原因、卖出原因、PnL、失败点和下一轮策略调整。

### 48.3 Operator 工作流优先级

下一阶段页面和脚本只服务操作员，不服务普通用户转化：

- `Projects / Overview` 要快速回答“现在是否值得买、买多少、主要风险是什么”。
- 自动买入/自动卖出控制台只保留实盘需要的参数和状态。
- 执行链路优先保证 nonce、fee、gas、allowance、balance、receipt、fuse 和失败恢复。
- 每个真实 live 窗口必须产出可复盘 archive，不再依赖零散生产日志。
- 新增功能优先级必须按“能否提高下一次实盘胜率或降低执行失败率”排序。

### 48.4 下一批任务边界

- 继续使用 `main` 作为当前主线，不再为 Base 生态方向单独延展分支。
- 高风险实验仍然先做 checkpoint commit；重写执行器、切换 RPC 架构或引入用户授权合约时再考虑单独分支。
- 生产同步继续遵守 `deploy_production_safe.sh --dry-run` 后再 `--apply`。
- Operator 实盘工作流和下一次真实 live 项目操作清单沉淀在 `docs/phases/phase-061-operator-profit-workflow.md`；后续 Phase 061 的代码/UI 优先级都以该 runbook 为准。
- 2026-05-20 已从生产 SignalHub upcoming feed 选出 `PROFIT` 与 `MTR` 两个候选，并完成非广播预热模拟；候选记录见 `docs/phases/phase-061-live-candidates-2026-05-20.md`。当前结论是 `MTR` 更适合作为第一次完整 runbook 演练，且已创建只读 `dryrun,prewarm` timer；真实广播前仍必须单独确认预算、授权和 timer 服务范围。
