...
- [x] 2026-06-01 将“跟单买入”从大户榜单策略卡中拆出，改为与“含税估算 FDV 限价单”平级的独立板块；跟单策略不叠加大户榜单 / 税率 / FDV / 横盘跳过 / 抄底条件。
- [x] 2026-06-01 收敛跟单买入前端文案：删除“参与触发”和“保存跟单设置”，改为 `启用跟单 / 停用跟单 / 保存跟单参数`。
- [x] 2026-06-02 三套买入预算拆分：大户榜单 `maxProjectV`、跟单 `followMaxProjectV`、含税估算 FDV 限价单 `fdvLimitMaxProjectV` 独立保存、独立热加载、独立统计已买入 V；前端删除“共享项目预算”语义。
...
- [x] 支持编辑基础买入、抄底买入、抄底阈值、横盘暂停阈值、单笔上限和大户榜单预算。
...
- [x] 项目预算前端改为三套独立预算语义：普通大户策略、跟单买入、含税估算 FDV 限价单分别消耗自己的预算，避免互相占用。
- [x] `launch_prewarm_executor.py` 支持配置热加载。
- [x] 配置缺失时金额沿用 CLI/systemd 默认值，但预算 scope 仍按策略独立。
- [x] `enabled=false` 或 `mode=simulate` 时阻断真实买入。
- [x] 配置版本变化时写入 `strategy_config_reloaded`。
- [x] 静态风控校验：买入金额不能超过单笔上限，大户榜单/跟单/限价单预算分别不能低于各自已发送买入 V。
